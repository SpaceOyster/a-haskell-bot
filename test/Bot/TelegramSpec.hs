...
errorWith :: Int -> T.Text -> ByteString
errorWith code txt =
  A.encode $
    A.object
      [ "ok" .= False,
  ...
  describe "fetchUpdates" $ do
    context "polls Telegram API for Updates list and converts it to [Bot.Entity]" $
      prop "result list is not longer then recieved Updates list" $
        \(tgCfg, updates) -> do
          let reply = A.encode . TG.ResponseWith $ A.toJSON (updates :: [TG.Update])
...
getAuthorsSettingsSpec =
  describe "getAuthorsSettings" $ do
  ...
  describe "echoMessageNTimes" $ do
    ...
    context "when error happens" $
      prop "catches and logs error" $ \(tgCfg, tgMsg, Positive n, err) -> do
        let repl = A.encode $ TG.ErrorResponse err
...
execCommandSpec =
  ...
  where
    ...
    propForWith cmd repl = \(tgCfg, tgMsg, err) -> do
      ...
      let apiErr = A.encode $ TG.ErrorResponse err
